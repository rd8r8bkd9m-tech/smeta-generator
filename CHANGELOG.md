...
#### Backend (API)
- Автосоздание демо-пользователя при отсутствии userId
- Гибкая привязка клиентов/проектов без обязательной авторизации

#### Frontend (Web)
- Полный CRUD клиентов и проектов через API (без мок-данных)
- Сохранение смет в базу данных с привязкой к проекту
- Автосохранение черновиков смет в localStorage
- Загрузка смет по `estimateId` с возможностью редактирования

### Изменено

- Обновлены ссылки на редактирование смет в карточке проекта
- Улучшены сценарии работы с реальными данными (без демо-стабов)
